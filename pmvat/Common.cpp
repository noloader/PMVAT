...
SquatParams::SquatParams()
: m_state(None), m_pid(PROCESS_ID_INVALID), m_pos(-1), m_number(-1),
m_thread(NULL), m_cancel( NULL ), m_release( NULL ), m_abandon( NULL )
{
    // Use Ref() to get to the raw handle. Otherwise, operator= will duplicate
    m_cancel.Ref() = CreateEvent( NULL, TRUE, FALSE, NULL );
    m_release.Ref() = CreateEvent( NULL, TRUE, FALSE, NULL );
    m_abandon.Ref() = CreateEvent( NULL, TRUE, FALSE, NULL );
}

SquatParams::~SquatParams()
{
    m_thread.Close(); m_thread = NULL;
    m_cancel.Close(); m_cancel = NULL;
    m_release.Close(); m_release = NULL;
    m_abandon.Close(); m_abandon = NULL;
}
...
wstring MutexStateAsString(HANDLE handle)
{
    ...
    DWORD dwWait = TestMutex(handle);
    switch( dwWait )
    {
    case WAIT_OBJECT_0:
        state = L"Signaled"; break;
    case WAIT_ABANDONED:
        state = L"Abandoned"; break;
    case WAIT_TIMEOUT:
