...
BEGIN_MESSAGE_MAP(CInfoView, CEditView)
    ...
    ON_WM_SIZE()
    ON_WM_CONTEXTMENU()
    ON_COMMAND(ID_EDIT_COPY, &CInfoView::OnEditCopy)
    ON_UPDATE_COMMAND_UI(ID_EDIT_COPY, &CInfoView::OnUpdateEditCopy)
END_MESSAGE_MAP()
...
CPmvatDoc* CInfoView::GetDocument()
{
...
BOOL CInfoView::UpdateVersionInformation()
{
    ...
    if( pDoc->m_pid == PROCESS_ID_INVALID )
    {
...
BOOL CInfoView::DisplayModuleInformation()
{
    ...
    CString str, res, strName = pDoc->m_name;
    INT pid = pDoc->m_pid;
...
void CInfoView::OnUpdateEditCopy(CCmdUI *pCmdUI)
{
