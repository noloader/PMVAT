...
BEGIN_MESSAGE_MAP(CLogView, CEditView)
    ...
    ON_COMMAND(ID_EDIT_COPY, &CLogView::OnEditCopy)
    ON_UPDATE_COMMAND_UI(ID_EDIT_COPY, &CLogView::OnUpdateEditCopy)
END_MESSAGE_MAP()
...
CPmvatDoc* CLogView::GetDocument()
{
    ...
    case HINT_LOG_MESSAGE:
        {
            ...
            if( NULL != pHint ) {
                ...
                delete ((CString*)pHint); pHint = NULL;
            }
            break;
        }
    default:
...
void CLogView::OnUpdateEditCopy(CCmdUI *pCmdUI)
{
    if( GetEditCtrl().GetWindowTextLength() )
    {
        pCmdUI->Enable(TRUE);
