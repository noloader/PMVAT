...
extern volatile bool g_bEulaAccepted;
...
CPmvatDoc* CProcTreeView::GetDocument()
{
...
void CProcTreeView::OnTvnSelChanged(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    *pResult = !PROCESSED;
    ...
    if( m_currentPid != previousPid )
    {
        ...
        pDoc->ProcessChanged( m_listProcs[idx].szName, m_listProcs[idx].nProcessId );
        pDoc->UpdateAllViews( this, HINT_PROCESS_CHANGED, NULL );
    }
...
UINT CDECL MonitorThreadProc( LPVOID lpParams )
{
    ...
    for( ; ; )
    {
        ...
        do
        {
            // No monitoring if not licensed
            if( !g_bEulaAccepted ) { break; }
            ...
            if( bChanged ) {
                if( hWnd ) {
                    SendMessage( hWnd, MSG_PROCESS_LIST_CHANGED, NULL, NULL );
                }
            }
